...
import { blocksAPI, exportAPI } from '../lib/api';
import { format } from 'date-fns';
import { Calendar, MapPin, Users, Clock, ArrowLeft, Filter, Download, Wheat } from 'lucide-react';

export default function BlockActivityHistory() {
  const { id } = useParams();
  ...
  const loadData = async () => {
    try {
      const blockRes = await blocksAPI.get(id);
  ...
  const loadHistory = async () => {
    ...
    try {
      ...
      const response = await blocksAPI.getActivityHistory(id, params);
  ...
  const handleExport = async () => {
    try {
      ...
      link.href = url;
      link.setAttribute('download', `Block_${block?.name}_activity_${new Date().toISOString().split('T')[0]}.xlsx`);
